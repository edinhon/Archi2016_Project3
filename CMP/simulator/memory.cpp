...
char memory::getData(unsigned int dataAddress, D_page_table *dpt, D_TLB *dtlb, int counter){
	...
	//TLB Miss
		...
		//Page Table Miss
			...
			moveFromDiskToMemory(counter, physical_page_number, dataAddress);
...
void memory::moveFromDiskToMemory(int counter, unsigned int physical_page_number, unsigned int PC){
	int len = D_page_size;
	D_memory[physical_page_number].valid = true;
	D_memory[physical_page_number].usedPCCycle = counter;
	for(int i = 0 ; i < len ; i++){
		D_memory[physical_page_number].memory[i] = D_disk[PC + i];
	}
}
...
void memory::deleteCacheInOriginMemory(unsigned int physical_page_number){
	...
	for(int i = 0 ; i < index_number ; i++){
		for(int j = 0 ; j < n_way ; j++){
			thisPPN = (D_cache_set[i].D_cache_block[j].tag * index_number * D_block_size) + (i * D_block_size);
